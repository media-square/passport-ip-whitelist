...
Strategy.prototype.authenticate = function (req, options) {
    ...
    if (!settings.users) {
        return this.fail(new BadRequestError('No IP whitelist provided'), 400);
    }
    else if (!(settings.users instanceof Array)) {
        return this.fail(new BadRequestError('Provided IP whitelist is not an array'), 400);
    }
    else if (!remoteIp) {
        return this.fail(new BadRequestError('Can\'t find remote IP address in request'), 400);
    }
    const user = findUser(remoteIp, settings.users);
    if (!user) {
        return this.fail(new BadRequestError(settings.badRequestMessage || 'Remote IP address not authenticated'), 400);
    }
    if(settings.tokenValidation){
        const requestToken = req.headers.token || req.query.token || req.body.token || false;
        if( !requestToken){
            return this.fail(new BadRequestError(settings.badRequestMessage || 'Token is required'), 400);
        }
        if(user.token !== requestToken){
            return this.fail(new BadRequestError(settings.badRequestMessage || 'Provided token is invalid'), 400);
        }
    }
